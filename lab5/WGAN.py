...
def train(G, D, epoch, device):
    g_optimizer = optim.RMSprop(G.parameters(), lr=rg, alpha=0.5)
    d_optimizer = optim.RMSprop(D.parameters(), lr=rd, alpha=0.5)
    g_optimizer = optim.Adam(G.parameters(), lr=rg)
    d_optimizer = optim.Adam(D.parameters(), lr=rd)
    name = 'weightWGANAdam'
    pathG = './'+name+'G.tar'
    pathD = './'+name+'D.tar'
    initepoch = 0
    ...
    for e in range(initepoch, epoch):
        ...
        if e % 10 == 9 or e == 0:
            test(e)
        print("epoch %d: cost %.3f s" %(e, time.time()-starttime))
