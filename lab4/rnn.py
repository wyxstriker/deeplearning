...
class MyRNN(torch.nn.Module):
    def __init__(self):
        ...
        self.rnncell = torch.nn.RNNCell(64, 10)
    ...
    def init_hidden(self):
        return torch.zeros(batchsize, 10).to(device)
...
if __name__ == '__main__':
    ...
    # writer = SummaryWriter('/output/rnn2')
    model = Model()
    model.to(device)
    #     train(epoch, model, device)
    checkpoint = torch.load('./weightsRNN.tar', map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.eval()
    test(model, device)
    # writer.close()
