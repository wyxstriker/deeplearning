...
class MyBiLSTM(torch.nn.Module):
    def __init__(self):
        ...
        self.rnncell1 = torch.nn.LSTMCell(64, 10)
        self.rnncell2 = torch.nn.LSTMCell(64, 10)
    ...
    def forward(self, x):
        ...
        output2 = None
        for i in range(seq):
            h1 = self.rnncell1(x[i], h1)
            if output1 is None:
                output1 = h1[0].unsqueeze(0)
            else:
                output1 = torch.cat([output1,h1[0].unsqueeze(0)], dim=0)
        for i in range(seq-1, -1, -1):
            h2 = self.rnncell2(x[i], h2)
            if output2 is None:
                output2 = h2[0].unsqueeze(0)
            else:
                output2 = torch.cat([output2,h2[0].unsqueeze(0)], dim=0)
        output1 = output1.transpose(1, 0)
        output2 = output2.transpose(1, 0)
        output = torch.cat([output1, output2],dim=2)
        h = torch.cat([h1[0],h2[0]],dim=1)
        return output, h

    def init_hidden(self):
        return (torch.zeros(batchsize, 10).to(device), torch.zeros(batchsize, 10).to(device))
        # return torch.zeros(batchsize, 10).cuda()
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.liner1 = torch.nn.Linear(4000,1024)
    ...
    def forward(self, x):
        x = self.emb(x)
        output, h = self.rnn(x)
        output = output.contiguous().view(-1, 4000)
        x = self.drop(x)
...
if __name__ == '__main__':
    ...
    # writer = SummaryWriter('/output/dropout1/')
    model = Model()
    model.to(device)
    #     train(epoch, model, device)
    checkpoint = torch.load('./weightsBiLSTM.tar', map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])
    test(model, device)
    # writer.close()
